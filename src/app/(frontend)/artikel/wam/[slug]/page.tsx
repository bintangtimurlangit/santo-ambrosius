...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = await getWAMBySlug(slug)

  if (!article) {
    return {
      title: 'WAM Tidak Ditemukan',
      description: 'WAM yang Anda cari tidak ditemukan.',
      robots: {
        index: false,
        follow: false,
      },
  ...
  return {
    title,
    description,
    keywords: ['wam', 'warta ambrosius mingguan', 'santo ambrosius', 'gereja katolik'],
    authors: [{ name: 'Tim Redaksi Santo Ambrosius' }],
    openGraph: {
      title,
      description,
      url: `/artikel/wam/${slug}`,
      type: 'article',
          ...
          <div className="py-16 md:py-20 lg:py-24">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center">
                ...
                <p className="text-slate-600 mb-8">Maaf, file PDF untuk WAM ini belum tersedia.</p>
                <Link
                  href="/artikel"
